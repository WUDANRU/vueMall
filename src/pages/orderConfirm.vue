        ...
        <div class="order-box">
          <div class="item-address">
            ...
            <div class="addr-list clearfix">
             <div class="addr-info" :class="{'checked':index == checkIndex}" @click="checkIndex=index" v-for="(item,index) in list" :key="index">  <!-- @click="checkIndex=index" 当点击了,checkIndex默认是0，但是index会吧它的值附给checkIndex，checkIndex就会改变值 -->
...
export default{
  ...
  mounted(){
    this.getAddressList(); //收货地址列表 //重新拉取一次，防止两个人同时登录删除不同的数据，xhr走了接口'/shippings重新拉取
    this.getCartList();
  },
  methods:{
    ...
    getCartList(){
      this.axios.get('/carts').then((res)=>{
        ...
        this.cartList = list.filter(item=>item.productSelected); //购物车列表勾选的
        this.cartList.map((item)=>{//因为是数组，所以要+=
    ...
    // 订单提交
    orderSubmit(){
      let item = this.list[this.checkIndex]; //收货地址
      if(!item){
      ...
      this.axios.post('/orders',{
        ...
        this.$router.push({
          ...
          query:{
            orderNo:res.orderNo 
            //这个会传到订单结算页面去  res.orderNo返回的订单号  当购物车页面cart点击结算跳转到订单确认页面confirm，
            // 再点击跳转到/#/order/pay?orderNo=1589529484958,返回到购物车页面cart，将显示是空的
          } //如果没有选中商品不会跳转到新页面
        })
      })
    }
